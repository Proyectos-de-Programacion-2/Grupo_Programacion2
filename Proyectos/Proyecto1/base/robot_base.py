...
class RobotBase:
    def __init__(self, nombre, capacidad_carga, x_inicial=0.0,y_inicial=0.0, yaw_inicial=0.0):
        ...
        def step(self, v, w):
            ...
            x_nuevo = self.__pos_x + v*np.cos(yaw_nuevo_norm)*self.__setp_dt
            y_nuevo = self.__pos_y + v*np.cos(yaw_nuevo_norm)*self.__setp_dt

            self._actualizar_pose(x_nuevo, y_nuevo, yaw_nuevo_norm)

            distancia = calc_dist_to_goal(self.__pos_X, self.__pos_y, self.target_x, self._target_y)
            error_angular = calc_yaw_error(self.__pos_x, self.__pos_y, self.__yaw, self.target_x, self.target_y)
